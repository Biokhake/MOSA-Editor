    ...
    if (style === 0) {
      ...
      for (const a of [0.25, 0.75, 1.25, 1.75]) {
        out.push(
          C(
            ...
            0.012 * t,
            0.052,
            Math.cos(a * Math.PI) * 0.088 * t,
            0.002,
            CZ + Math.sin(a * Math.PI) * 0.088 * t,
            0,
            0,
            0,
            6,
      ...
      if (r.ornate)
        for (const sgn of [-1, 1] as const)
          out.push(C("metal", 0.01, 0.01, 0.05, sgn * bar, 0.02, CZ, 0, 0, 0, 6));
    }
  } else {
    // ================= R-CLASS (curved border) =================
    if (style === 0) {
      // R0 — single torus ring
      out.push(Torus(rimMat, 0.078 * t, 0.026, 0, 0.002, CZ, Math.PI / 2, 0, 0, Math.max(18, s)));
      if (r.ornate)
        out.push(Torus("trim", 0.082 * t, 0.009, 0, 0.018, CZ, Math.PI / 2, 0, 0, Math.max(20, s)));
    } else if (style === 1) {
      ...
      if (r.ornate)
        out.push(C("sec", 0.05 * t, 0.05 * t, 0.006, 0, 0.026, CZ, 0, 0, 0, Math.max(16, s)));
    } else {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        out.push(
          Sp("metal", 0.014 * t, Math.cos(a) * 0.088 * t, 0.002, CZ + Math.sin(a) * 0.088 * t, 8),
        );
      }
      if (r.ornate)
        out.push(Torus("trim", 0.078 * t, 0.007, 0, 0.02, CZ, Math.PI / 2, 0, 0, Math.max(20, s)));
    }
  }
...
export function cockpit(r: Recipe): Spec[] {
  ...
  if (r.ornate) {
    out.push(B("sec", 0.1 * t, 0.12, 0.04, 0, 0.01, 0.08, 0.25, 0, 0));
  }
...
export function abdomen(r: Recipe): Spec[] {
  ...
  out.push(Torus("dark", 0.08 * t, 0.01, 0, 0, -0.04, 0, Math.PI / 2, 0));
...
export function pelvis(r: Recipe): Spec[] {
  ...
  out.push(C("metal", 0.018, 0.018, 0.3 * t, 0, -0.03, 0, 0, 0, Math.PI / 2, 8));

  if (r.ornate) {
    out.push(B("trim", 0.26 * t, 0.018, 0.16, 0, 0.05, 0));
  }
...
export function skirtB(r: Recipe): Spec[] {
  ...
  out.push(
    ...
    Trap("prim", 0.2 * t, 0.15, 0.035, 0, -0.03, -0.04, 0.25, 0, 0),
  );
