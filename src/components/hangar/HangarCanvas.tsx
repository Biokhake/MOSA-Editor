...
import type { ReconstructedKit, ReconstructedSegment } from "@/lib/mech/reconstruction/types";
import {
  buildCompoundSegment,
  disposeCompound,
  segmentCentroid,
} from "@/lib/mech/reconstruction/geometry";
...
function CompoundSegmentMesh({
  ...
  const group = useMemo(
    () => buildCompoundSegment(segment, kit, paint, paint2, light, edges, theme),
    [segment, kit, paint, paint2, light, edges, theme],
  );
  useEffect(() => () => disposeCompound(group), [group]);
  return <primitive object={group} />;
}
...
function MechRig() {
  ...
  const segmentsByNode = useMemo(() => {
    const map = new Map<RigNodeId, ReconstructedSegment[]>();
    for (const segment of reconstructedKit?.segments ?? []) {
      (map.get(segment.anchor) ?? map.set(segment.anchor, []).get(segment.anchor)!).push(segment);
    }
    return map;
  }, [reconstructedKit]);
  ...
  const renderNode = (id: RigNodeId) => {
    ...
    return (
      <group key={id} position={position} rotation={rotation} scale={scale}>
        ...
        {reconstructedKit &&
          (segmentsByNode.get(id) ?? []).map((segment) => {
            const st = slots[segment.id];
            if (!st || !st.visible) return null;
            const center = segmentCentroid(segment, reconstructedKit);
            const [dx, dy, dz] = explodeDir(center);
            const k = explode * 0.85;
            return (
              <group
                key={segment.id}
                userData={{ slotId: segment.id }}
                position={[st.px + dx * k, st.py + dy * k, st.pz + dz * k]}
                rotation={[st.rx * DEG, st.ry * DEG, st.rz * DEG]}
                scale={[st.sx, st.sy, st.sz]}
              >
                <CompoundSegmentMesh
                  segment={segment}
                  kit={reconstructedKit}
...
export function HangarCanvas() {
  ...
  if (!webglAvailable) {
    return (
      <div
        className="absolute inset-0 grid place-items-center bg-bg px-8 text-center"
        role="status"
      >
        <div className="max-w-sm">
          <p className="font-display text-sm tracking-wide text-fg">3D preview unavailable</p>
          <p className="mt-2 font-mono text-xs leading-relaxed text-muted">
            This browser has WebGL disabled. Part data and editor controls remain available.
          </p>
        </div>
      </div>
    );
      ...
      <Suspense fallback={null}>
        ...
        <EffectComposer multisampling={2} enableNormalPass>
          ...
          <N8AO
            ...
            halfRes
          />
          <Bloom luminanceThreshold={1.15} luminanceSmoothing={0.12} intensity={0.7} mipmapBlur />
